import {dzsUploaderTargetInit} from "./_uploaderPreview";
export function setup_autoClick(){
...
export function reskin_select_setup() {

  reskin_select();
  setTimeout(reskin_select, 10);
  setTimeout(function () {

    jQuery('select.vpconfig-select').trigger('change');
  }, 1000);
}
...
export function show_feedback(arg, pargs) {
  ...
  var feedbackerClass = 'feedbacker ' + margs.extra_class;
  ...
  if (margs.extra_class == '') {
    ...
    if (arg.indexOf('error - ') == 0) {
      arg = arg.substr(8);
      feedbackerClass = 'feedbacker is-error';
    }
  }

  _feedbacker.attr('class', feedbackerClass);
