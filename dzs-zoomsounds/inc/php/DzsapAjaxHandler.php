...
class DzsapAjaxHandler {
  ...
  function register_actions() {
	...
	// Replace admin_nag_disable_all() with:
	function admin_nag_disable_all() {
		// Verify nonce (AJAX)
		if ( ! isset( $_REQUEST['security'] ) || ! wp_verify_nonce( $_REQUEST['security'], 'dzsap_admin_action_nonce' ) ) {
			wp_send_json_error( 'invalid_nonce', 403 );
		}

		// Capability check - restrict to administrators (or choose appropriate capability)
  ...
  function get_metaViews($postId) {
	...
	function admin_report_connect_zoomsounds() {
		...
		if ( ! function_exists( 'wp_remote_get' ) ) {
			wp_send_json_error( 'remote_unavailable', 500 );
		}

		$request = wp_remote_get( DZSAP_ADMIN_UPDATE_LATEST_VERSION_URI, array( 'timeout' => 10 ) );
		$code = wp_remote_retrieve_response_code( $request );
		$body = wp_remote_retrieve_body( $request );

		if ( is_wp_error( $request ) || $code !== 200 || empty( $body ) ) {
			wp_send_json_error( 'remote_fetch_failed', 502 );
		}

		// Sanitize/allow only safe HTML if necessary. If expecting JSON, decode and validate instead.
		$safe = wp_kses_post( $body );

		wp_send_json_success( array( 'payload' => $safe ) );
	}
...
//      echo json_encode($responseArray);
  ...
  function activate_license() {
	...
	function hide_intro_nag() {
		...
		$this->dzsap->mainoptions['intro_nag_hidden'] = 'on';
		update_option( DZSAP_DBNAME_MAINOPTIONS, $this->dzsap->mainoptions, false );
