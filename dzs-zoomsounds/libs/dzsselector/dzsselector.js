...
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.CLASS_VIEW_TRIGGER_CHANGE_ON_REINIT=exports.CLASS_VIEW_TRIGGER_CHANGE=exports.OPENER_TYPE=exports.CLASS_BIGOPTION_SELECTED=exports.SELECTOR_TYPE=exports.CLASS_FEEDER=exports.CLASS_OPTION_LABEL=exports.CLASS_PLACEHOLDER_OPTION=exports.CLASS_SELECT_IS_SELF_FEEDER=void 0;const CLASS_SELECT_IS_SELF_FEEDER="select-is-self-feeder";exports.CLASS_SELECT_IS_SELF_FEEDER="select-is-self-feeder";const CLASS_PLACEHOLDER_OPTION=".feed-dzswtl--placeholder";exports.CLASS_PLACEHOLDER_OPTION=CLASS_PLACEHOLDER_OPTION;const CLASS_OPTION_LABEL=".the-label--name";exports.CLASS_OPTION_LABEL=".the-label--name";const CLASS_FEEDER="dzs-style-me-feeder";exports.CLASS_FEEDER=CLASS_FEEDER;const SELECTOR_TYPE={SELECT:"select",MULTIPLE_CHECKBOXES:"multiple-checkboxes"};exports.SELECTOR_TYPE=SELECTOR_TYPE;const CLASS_BIGOPTION_SELECTED="active-checked";exports.CLASS_BIGOPTION_SELECTED="active-checked";const OPENER_TYPE={BIGOPTIONS:"opener-bigoptions",LISTBUTTONS:"opener-listbuttons",RADIO:"opener-radio",LIST:"opener-list"};exports.OPENER_TYPE=OPENER_TYPE;const CLASS_VIEW_TRIGGER_CHANGE="do-not-trigger-change-on-init";exports.CLASS_VIEW_TRIGGER_CHANGE=CLASS_VIEW_TRIGGER_CHANGE;const CLASS_VIEW_TRIGGER_CHANGE_ON_REINIT="do-not-trigger-change-on-reinit";exports.CLASS_VIEW_TRIGGER_CHANGE_ON_REINIT="do-not-trigger-change-on-reinit";
 ...
 * @property {jQuery} $selectInputForm - the select element ( if it exists )
...
class DzsSelector {
  ...
  classInit() {
          ...
          selfInstance.$selectInputForm.children().each(function () {
            ...
            if (_t2.prop('selected')) {
              if (selfInstance.$openerWrap) {
                selfInstance.$openerWrap.children().removeClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
                selfInstance.$openerWrap.children().eq(ind).addClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
              }

              selfInstance.view_selectWrapperHeaderSetLabel(_t2.html());
              return false;
            }
    ...
    function reinit() {
      ...
      if (selfInstance.isOpener) {
        ...
        (0, _viewOpenerArea.opener_selectFromSelectedIndex)(selfInstance);
      }
    }
  }

  view_getSelectedHtmlFromInput() {
    let selectedOptionHtml = '';
    const $selectedHtml = this.$selectInputForm.find(':selected');
    selectedOptionHtml = $selectedHtml.html();
    return {
      selectedOptionHtml,
      $selectedHtml
    };
  ...
  view_selectWrapperHeaderSetLabel(newHtml) {
...
exports.opener_wrapperHeadConstructStringSelectedOptions = opener_wrapperHeadConstructStringSelectedOptions;
...
function opener_reinit_checkMultipleActiveOptions(selfInstance) {
  ...
  selfInstance.$openerMain.find('input[type="checkbox"]').each(function () {
    ...
    if ($t.prop('checked')) {
      ...
      if ($t.parent().hasClass('multiple-option') && $t.parent().hasClass(_selectorConfig.CLASS_BIGOPTION_SELECTED)) {
        const ind = $t.parent().parent().children().index($t.parent());
        isGoingToUpdateLabel = true;
...
function opener_wrapperHeadConstructStringSelectedOptions(selfInstance) {
  ...
  if (selfInstance.isOpener) {
    selfInstance.$openerMain.children().each(function () {
      ...
      if ($openerMainChild.hasClass(_selectorConfig.CLASS_BIGOPTION_SELECTED)) {
...
function opener_open(selfInstance) {
  return function () {
    ...
    const isOpenerActive = selfInstance.$wrapperMain.hasClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
    ...
    selfInstance.$wrapperMain.addClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
    setTimeout(function () {
    ...
    if (o.opener === _selectorConfig.OPENER_TYPE['LIST']) {}
  };
}

function opener_clearActiveChecked(selfInstance) {
  selfInstance.$openerMain.children().removeClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
}
/**
 *
 * @param {DzsSelector} selfInstance
...
function opener_selectOption(selfInstance, currentIndexSelected, forceIsActive) {
  ...
  if ($openerChildAtIndex.hasClass(_selectorConfig.CLASS_BIGOPTION_SELECTED)) {
  ...
  if (!currentOptionIsChecked) {
    $openerChildAtIndex.addClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
  } else {
    // -- current option is already checked
    if (selfInstance.selectorOptions.type === _selectorConfig.SELECTOR_TYPE.MULTIPLE_CHECKBOXES) {
      $openerChildAtIndex.removeClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
    }
...
function opener_close(selfInstance) {
  return function () {
    ...
    setTimeout(function () {
      selfInstance.$wrapperMain.removeClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
    }, delay);

    if (String(selfInstance.feedElementClass).indexOf(_selectorConfig.OPENER_TYPE['BIGOPTIONS']) > -1) {
      ;
    }
...
function setup_listeners(selfInstance) {
  ...
  if (selfInstance.$selectInputForm) {
    ...
    if (!selfInstance.$selectInputForm.hasClass(_selectorConfig.CLASS_VIEW_TRIGGER_CHANGE)) {
      selfInstance.$selectInputForm.trigger('change');
    }
  }

  if (selfInstance.isOpener) {
    const $wrapperHead = selfInstance.$dzsSelectorMain.parent().find('.dzs-select-wrapper-head');
  ...
  if (o.opener === _selectorConfig.OPENER_TYPE['LISTBUTTONS'] || o.opener === _selectorConfig.OPENER_TYPE['BIGOPTIONS'] || o.opener === _selectorConfig.OPENER_TYPE['RADIO'] || o.opener === _selectorConfig.OPENER_TYPE['LIST']) {
    ...
    if (e.type === 'click') {
      if (_t.hasClass('select-option')) {
        if (!window.dzssel_click_triggered) {
          const $selectedOpenerOption = $(this); // -- active
          // -- if it is select, remove other selected

          const currentIndexSelected = $selectedOpenerOption.parent().children('.select-option').index($selectedOpenerOption);
          ...
          if (selfInstance.$selectInputForm && selfInstance.typeSelectIsAllowEmptyOptions !== true) {
            selfInstance.$selectInputForm.trigger('change');
          } else {
            // -- no select input form
            (0, _openerFunctions.opener_selectOption)(selfInstance, currentIndexSelected, undefined);
          }

          const string_currentSelected = (0, _openerFunctions.opener_wrapperHeadConstructStringSelectedOptions)(selfInstance);
          selfInstance.view_selectWrapperHeaderSetLabel(string_currentSelected);
  ...
  function handle_changeSelectInputForm(selfInstance) {
    return function () {
      ...
      if (selfInstance.$selectInputForm.length) {
        const $selectHtml = selfInstance.view_getSelectedHtmlFromInput();
        const $selectedHtml = $selectHtml.$selectedHtml;
        selectedOptionHtml = $selectHtml.selectedOptionHtml;
        currentIndexSelected = selfInstance.$selectInputForm.children().index($selectedHtml);

        if (selfInstance.isOpener) {
          (0, _openerFunctions.opener_selectOption)(selfInstance, currentIndexSelected, false);
        } else {
          selfInstance.view_selectWrapperHeaderSetLabel(selectedOptionHtml);
        }
      }
    };
  }
...
},{"../configs/_selectorConfig":1,"./_opener-functions":4,"./_selector-functions":6,"./_view-functions":8}],6:[function(require,module,exports){
"use strict";
...
function selector_initial_setup(selfInstance) {
  ...
  if (selfInstance.$dzsSelectorMain.hasClass(_selectorConfig.OPENER_TYPE['LISTBUTTONS'])) {
    o.opener = _selectorConfig.OPENER_TYPE['LISTBUTTONS'];
    selfInstance.isOpener = true;
  }

  if (selfInstance.$dzsSelectorMain.hasClass(_selectorConfig.OPENER_TYPE['BIGOPTIONS'])) {
    o.opener = _selectorConfig.OPENER_TYPE['BIGOPTIONS'];
    selfInstance.isOpener = true;
  }

  if (selfInstance.$dzsSelectorMain.hasClass(_selectorConfig.OPENER_TYPE['RADIO'])) {
    o.opener = _selectorConfig.OPENER_TYPE['RADIO'];
    selfInstance.isOpener = true;
  }

  if (selfInstance.$dzsSelectorMain.hasClass(_selectorConfig.OPENER_TYPE['LIST'])) {
    o.opener = _selectorConfig.OPENER_TYPE['LIST'];
    selfInstance.isOpener = true;
  }
...
exports.opener_determineSelected = opener_determineSelected;
exports.opener_clearBigOptions = opener_clearBigOptions;
exports.opener_selectFromSelectedIndex = opener_selectFromSelectedIndex;
...
function opener_setupStructure(selfInstance) {
  ...
  if (o.opener === _selectorConfig.OPENER_TYPE['LIST'] || o.opener === _selectorConfig.OPENER_TYPE['BIGOPTIONS']) {
    // -- we just need opener-list for search
    if (o.opener === _selectorConfig.OPENER_TYPE['LIST'] && !selfInstance.$dzsSelectorMain.parent().hasClass('disable-search')) {
  ...
  if (o.opener === _selectorConfig.OPENER_TYPE['BIGOPTIONS']) {
...
function opener_determineSelected(selfInstance) {
  for (let i = 0; i < selfInstance.$dzsSelectorMain.children().length; i++) {
...
function opener_selectFromSelectedIndex(selfInstance) {
  if (selfInstance.selectorOptions.opener && selfInstance.selectorOptions.opener.indexOf('opener-') === 0) {
    if (selfInstance.$feederObject) {
      if (selfInstance.$openerMain) {
        selfInstance.$openerMain.find('.select-option').eq(selfInstance.selectedIndex).addClass(_selectorConfig.CLASS_BIGOPTION_SELECTED);
...
function opener_setupBigOptions(selfInstance) {
  ...
  if ($feederObject) {
    for (let i = 0; i < $feederObject.children().length; i++) {
      ...
      if ($feedObject.hasClass(_selectorConfig.CLASS_BIGOPTION_SELECTED)) {
        bigOptionExtraClasses += ' ' + _selectorConfig.CLASS_BIGOPTION_SELECTED;
      }

      bigOptionHtml = $feedObject.html();

      if (o.opener === _selectorConfig.OPENER_TYPE['RADIO']) {
  ...
  setTimeout(function () {
    if (selfInstance.$selectInputForm && !selfInstance.$selectInputForm.hasClass(_selectorConfig.CLASS_VIEW_TRIGGER_CHANGE_ON_REINIT)) {
      selfInstance.$selectInputForm.trigger('change');
    }
...
function opener_determineAreas(selfInstance) {
  ...
  if (selectorOptions.opener === _selectorConfig.OPENER_TYPE['BIGOPTIONS'] || selectorOptions.opener === _selectorConfig.OPENER_TYPE['LIST']) {
  ...
  if (selectorOptions.opener === _selectorConfig.OPENER_TYPE['LISTBUTTONS'] || selectorOptions.opener === _selectorConfig.OPENER_TYPE['RADIO']) {
    selfInstance.$openerWrap = selfInstance.$wrapperMain.find('.' + selectorOptions.opener + '-wrap');
