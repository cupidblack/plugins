...
?>">
  ...
  foreach ($dzsap->options_array_player as $categoryKey => $opt) {
    ...
    if (in_array($categoryKey, DZSAP_EXCLUDED_KEYS_FROM_GENERATOR_PLAYER)) {
      continue;
    }
...
' . dzsap_sliders_admin_generate_item_meta_cat('main', null, $argsShortcodeGenerator) . '
  ...
  foreach ($arr_examples as $example) {
    $fout .= '<div class="dzs-player-example" onClick=\'() => { console.log("dada"); }\' data-sample-name="' . $example['name'] . '">
